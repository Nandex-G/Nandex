class FlashCardMaker{
    ...
    searchMaker(options) {
        ...
        list.forEach(item => {
            searchElements[4].innerHTML +=  `<option value="${item.word.charAt(0).toUpperCase() + item.word.slice(1)}">`
        })
        ...
        function SearchMakerFunc() {
            if (!open && searchShow) {
                ...
                setTimeout(() => {
                    ...
                    searchElements[0].style.borderRadius = 0              
                }, 450);
                setTimeout(() => {
                ...
                setTimeout(() => {
                    searchElements[0].style.top = null
                    searchElements[0].style.width = null
                    searchElements[0].style.borderRadius = null
                }, 800); 
                setTimeout(() => {
                    searchElements[1].value = ''
                    searchElements[2].innerHTML = 'Cards will show here'
    ...
    searchCardShower(options) {
        let {searchElements , list , codeVerb , codeOther , codeNoun} = options
    ...
    resetEverything(option) {
        console.log(this.loadTimes  ,localStorage.getItem('wordsLIST') );
        this.cardBack = false
        this.cardNum = 0
        this.pageNum = 0
        this.canSlideNext = true
        ...
        if (this.loadTimes == 1) {
            ...
            option.list = JSON.parse(localStorage.getItem('wordsLIST')) 
            this.cardMaker(option)
        }
